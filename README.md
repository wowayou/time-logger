...
底层是**点存储 + 区间 UX**：每条记录只存一个起点 `ts`，段的结束由右邻记录（或结算点）实时派生，不存储区间。补录用「起点 + 终点」有界插入（`carveInsert`）：在某段内切出 `[起点, 终点)` 为新标签，并在终点自动补回原段标签、不牵连其它段；每段都带「补 / 切」入口。所有写路径（新增 / 编辑 / 删除 / 补录）末尾统一收敛到 `normalizeEntries`——去掉与相邻同标签的冗余边界，并恒保今天的尾占位条，因此增删改后不再被迫先 +1min。
...
- 离线可用（Service Worker 缓存全部运行时文件，首次联网加载后记录/编辑/统计/备份全程可离线；仅首次加载和版本更新需要联网）
