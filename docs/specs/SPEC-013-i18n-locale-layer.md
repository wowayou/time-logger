...
status: ready
owner: 执行方认领后填分支名 `spec/013-i18n-layer`
...
模块边界（CLAUDE.md 同款约束，写进文件头注释）：`i18n.js` **只做取词与 locale 解析，不访问 DOM、不写 localStorage 之外的任何存储、不 import 其它业务模块**（避免与 `storage.js` 形成环）。locale 的读写键是 `localStorage['timelog.locale']`，与 `timelog.v1`/`timelog.config` 并列，**不进备份**（语言是设备偏好，不是用户数据；导入不得改语言）。
...
- `audit_no_hardcoded_cjk_in_runtime`：`src/*.js`（除 `src/locales/`）与 `index.html` 的**非注释行**不得出现 CJK 字面量。
