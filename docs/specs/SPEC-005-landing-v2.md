...
1. **hero**：应用图标 + 「时间尺」 + 定位句（逐字用 CLAUDE.md 权威表述）+ 「打开应用」CTA + 「免费 · 无账号 · 数据只在你的设备上」副行。**图标必须复用应用自己的 `icon.svg`**（维护者 2026-07-24 明确要求，不要另画 τ 字符仿制品）：部署产物里引用 `./app/icon.svg`（build_site 布局下该路径必然存在），本地预览差异在 PR 里注明即可。
...
- 不写用户数、不写评价引语、不声称市场验证；「作者自用 N 天」是唯一可用的使用陈述。**这里锁的是形态不是数字**——数字随实际使用增长（v88 后由 `site/index.html` 的 `data-since` 每天自算，写稿当时的字面值只是下界），但不得换成用户数、留存率或任何第二人称证词。
