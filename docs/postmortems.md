...
> 记录故障发生时的证据、根因与修复。版本状态和临时处理步骤属于当时语境，不能替代当前维护规范；当前结果见 [STATUS.md](../STATUS.md)。

记录已发生并已修复（或已确诊待修）的功能/逻辑 bug：**现象 → 根因 → 修法 → 护栏 → 范围**。
与 `docs/decisions.md`（方向与取舍的决策）区分：这里只放真实出过问题的东西。
...
**2026-07-19 启动图生效确认（v66 缓解落地）**：维护者真机确认 v66 的 `apple-touch-startup-image`（SE 750×1334 暗色一张）**已生效**——冷孵化的纯黑段呈现品牌启动图，感知缓解按预期落地（纯观感、不缩短等待，与录屏判决的定位一致），不 revert。**不扩设备**：当前只有一台真实设备，为假想机型批量生成启动图属镀金；若未来出现第二台真实设备，按其精确尺寸补一张即可。
...
**随后已改掉（同日）**：判据从比值换成**增量的绝对毫秒预算**（`loaded − baseline`）。比值坏在分母又小又抖，而且随机器变快而升高——当初设计时基线 327–383ms，现在只有 83–133ms，同样健康的代码反而更容易撞线。改判据时又实测出两件事：① 机器忙时基线会被冷启动污染，出现过 `baseline=375ms loaded=156ms ratio=0.42x`（「装 2000 条比空数据快 2.4 倍」），那种轮次里断言恒真、**等于没测**——预热次数因此从 1 次加到 2 次；② 三个红灯探针实测出这道闸的**检出下限约是 5 倍劣化**（2500 万次异或的 O(n²) 只值 90ms，不触发也不该触发；O(n²) 且真做字符串比较的 delta 1191ms，触发）。**同时暴露一个覆盖缺口**：v87 那次真回归出在年视图聚合，而本用例量的是启动进日视图，那条路径根本不在测量范围内——探针复刻不出当年的量级不是探针的问题。守年视图的闸是 `v87_fixes.spec.js` 里那条结构性判据（一次 render 读了几次 config），两者互补，别把这里的绿灯读成「年视图也没事」。

---

## P38 · 配置配额失败造成跨 key 半事务（v90）

**确诊日期**：2026-08-13 · **状态**：v90 已修并发布（发布流水见 `CHANGELOG.md`） · **严重度**：高（配置动作会崩溃；标签迁移可造成记录归类失真）

**现象**：存储配额耗尽时，格言、长段提醒和标签配置等写入 `timelog.config` 的动作会抛出 `QuotaExceededError`，sheet 可能直接失去正常保存反馈。更严重的是标签改名/合并：流程先把迁移后的记录写入 `timelog.v1`，再写新配置；若第二次写入失败，记录已经换成新标签，配置却仍只认识旧标签，新标签随即掉进「未记录」桶，形成跨 key 半事务。

**根因**：`save()` 已把配额异常转成 `false`，`saveConfig()` 却直接调用 `localStorage.setItem()`，两条持久化 API 的失败契约不一致；调用者也因此没有补偿路径。

**为什么压测没早点抓到它**：已有 C 类压力测试只拦 `timelog.v1`（碰不到 config key），而且它用**给 `localStorage` 实例赋值**的方式 mock 原生方法。审核时用一次性探针（同时上报「拦截器有没有被调用」和「新记录有没有落库」两个事实）实测出**引擎分裂**：

```
                         拦截器被调用   新记录落库
Chromium  实例赋值            是           否      ← 故障确实注入
WebKit    实例赋值            否           是      ← 拦截器从未被调用
两引擎    Storage.prototype   是           否      ← 都注入成功
```

即：**这条用例在 WebKit 上终生空转**——绿灯，但配额路径一次都没走到，而它恰恰是唯一一条声称覆盖配额的用例。改拦 `Storage.prototype.setItem` 后两引擎都真注入。（`localStorage` 是 `Storage` 的实例，方法在原型上；给实例赋一个同名属性能否遮蔽它，取决于引擎对 `Storage` 这类 platform object 的属性定义方式——不是可以指望的行为。）

**修复**：`saveConfig()` 与 `save()` 统一识别 `QuotaExceededError` / `NS_ERROR_DOM_QUOTA_REACHED` 并返回失败。单独配置动作在失败时保留当前 sheet 和输入；导入沿用既有数据回滚。标签改名/合并以及「保存记录并记住自定义标签」属于跨 `timelog.v1` / `timelog.config` 的用户动作：在同一次 `load()` 后先保存写前快照，配置写失败就把记录补偿写回快照，再显示内联错误。补偿写覆盖已有 key，通常不增加配额占用；即使存储已满也比继续保留分裂状态可靠。

**护栏**：新增 `tests/v90_storage_pressure.spec.js`，双引擎覆盖标签改名失败后两个 key 均保持原值、格言失败后 sheet 与输入保留、自定义标签失败后新增记录回滚；并纳入 `npm run test:stress`。旧压力测试改为拦截 `Storage.prototype.setItem`，且只拒绝第一次目标写入，让后续补偿写真实执行。

**经验**：① 同一用户动作跨多个 localStorage key 时，必须明确提交顺序和补偿事务，不能把「每个 setItem 都是原子的」误当成「整个动作是原子的」；② 同类存储函数必须共享成功/失败契约；③ **注入故障的 mock 必须先证明故障真的注入了**——判据不能是「用例通过」，那与「什么都没发生」无法区分。可行的做法是让探针同时上报「拦截器被调用了吗」和「被保护的副作用发生了吗」两个事实，缺一不可：只看前者会漏掉「拦到了但代码绕过去了」，只看后者会漏掉「压根没拦到，而这条路径本来就不产生副作用」。④ 双引擎门禁的价值不只在于发现产品代码的引擎差异，也在于发现**测试自身**只在一个引擎上有效——本例正是后者，而它藏了很久。

---

## P39 · sheet 延迟聚焦抢走交互，连续保存偶发丢点击（v90）

**确诊日期**：2026-08-13 · **状态**：v90 已修并发布（发布流水见 `CHANGELOG.md`） · **严重度**：中（不损坏已有数据，但用户明确点击保存后修改可能没有落库）

**现象**：全量 UI 中「先保存自定义格言，再立即重开、清空并保存」偶发仍显示旧格言。最初像引擎时序波动，但隔离重复后 Chromium 20 次出现 1 次硬失败、3 次重试才过；把断言前移到第二次点击后的同步 `localStorage` 读取，又在 20 次里稳定抓到 2 次仍是旧值，证明第二次保存事件根本没有执行，而不是保存后渲染慢。

**根因**：`openFormSheet()` 每次打开都排一个 `requestAnimationFrame`，下一帧无条件给 `.form-sheet-panel` 加焦点。高负载时这帧可能迟到，用户已经聚焦输入框、甚至正在点击保存，它仍会把焦点抢回面板；连续关闭/重开时旧帧尤其容易跨过操作边界，最终吞掉点击。它与 v86 的 `pointerdown` 重渲染缺陷属于同一原则：**用户交互已经开始后，异步初始化不得再改激活目标**。

**修复**：延迟聚焦只在 sheet 仍打开、未进入关闭动画、且焦点仍在 sheet 外时执行。正常打开后面板仍是读屏/键盘的焦点起点；若用户先一步进入输入或按钮，则保留用户选择。

**护栏**：既有格言连续保存用例把第二次点击后的 config 断言前移，要求同步写入已经发生，不能再由后续 UI 等待掩盖丢点击。修复前 Chromium 20 次命中 2 次失败；修复后同一用例 Chromium/WebKit 各 20 次共 40/40 通过；面板初始聚焦、校验失败聚焦、快速点击自愈等相关双引擎用例 10/10 通过。

**经验**：① `requestAnimationFrame` 不是「马上」，负载下可能跨过一次完整交互；② 延迟设置焦点前必须重新检查用户是否已经把焦点放到组件内部；③ flaky 只描述复现频率，不是根因分类，能隔离复现就应继续查到确定的状态差异。
