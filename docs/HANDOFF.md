...
- **压测阈值用例 `stress.spec.js` 小压/极压在本机偶发红**（重试即过）：已用未改动代码在同一台机器复现同一条失败，与近期改动无关。它是边缘阈值用例，近期在 chromium/webkit 都飘过——**待单独收拾**（把绝对毫秒阈值改成相对基线，或加预热）。v79 实测数字：`boot=446ms` 对阈值 `450ms`，**余量 4ms（0.9%）**——飘不是偶然，是阈值本来就压在线上。
