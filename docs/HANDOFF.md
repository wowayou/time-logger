...
最后更新：2026-08-11（**v89 已发布上线**；随后两笔开发期改动：WebKit 代理修复 P37、stress 判据改绝对增量预算） · 更新人：Codex（实现）+ Claude Opus 5（验收、补批与发布，均为本地会话）

---

## 一句话现状

**v89 已发布上线**（合并 main + tag + Release + `publish-site` 绿灯，`time.eigentime.org/app/manifest.webmanifest` 实测 `89`）：默认移除长段确认门槛；高级设置保留「长段待核」本机开关，默认关闭，开启后只提醒、不改变原标签统计。依据与不可做项见 D25。v82–v88 均已发布并线上验证。**AI 侧无进行中任务、无 ready 规格**；唯一非 gated 的产品未完成项仍是 runbook `- [ ] E 完成`（首轮推广）——它不在 AI 侧。

**发布后又落了两笔纯开发期改动**（不影响运行时、不需要版本号）：WebKit 代理修复（P37）与 stress 判据改绝对增量预算，见下方两节。
