...
最后更新：2026-08-03（v82–v83 已发布上线） · 更新人：Claude Opus 5（本地会话）

---

## 一句话现状

**v82 / v83 已发布并线上验证**（tag + Release + `publish-site` 绿灯，`time.eigentime.org/app/manifest.webmanifest` 实测 `83`）。AI 侧**无进行中任务、无 ready 规格**（SPEC-001–015 全 done，SPEC-008 仍 park）。唯一非 gated 的未完成项仍是 runbook `- [ ] E 完成`（首轮推广）——**它不在 AI 侧**。下次复盘按事件触发（首轮推广后的第一批外部反馈，或第一个非维护者用户），不按日历（D16）。
...
4. **`.first()` 在标签设置里不安全**：主线分组出现后 `.cfg-name` 不唯一（v80 已改三处），v83 的草稿行让它更不唯一——一律按 `data-kind` / `data-original-name` / `data-new` 定位。
...
- **跨天记录的形态限制**（v77 记录）：数据层没有「跨天记录」，段＝[本点, 下一点)，午夜是统计边界；唯一为跨午夜设计的「过夜续记」要求昨天尾点是**未记录占位**。昨晚尾点若是真实记录（刷手机到睡着），想把「昨晚到今早都记成睡觉」只能两步手工完成。放宽触发条件＝改 v57 定下的形态判据，回归面大。
...
- **`使用与理念.md` 的「标签含义」段已过期**：仍在讲 `研究·学工具·逃避` / `杂` 那套旧标签，与现在的四桶 taxonomy 和默认种子对不上，也没提标签高级设置能改名 / 新建 / 删除。这是维护者的理念文档，AI 不擅自重写，**需要维护者定调后再改**。
