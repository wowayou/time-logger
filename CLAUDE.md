...
当前版本：`timelog-v51` / manifest `version: "51"`。
...
- v51 刷新接帧：每次主渲染后把 `.app` 与 FAB 的已转义 DOM 写入同标签页 `sessionStorage['timelog.bootSnapshot.v1']`；`index.html` 在 ES module 到达前同步恢复该快照并加 `.boot-restored`，避免 Safari 刷新先绘制空 HTML 壳。只有数据/config/视图/日期/记录模式/自然日仍与快照一致才恢复；快照不跨标签、不进备份、不替代 `localStorage` 权威数据。
...
- 导入冲突 UI 禁止暴露内部 ID 或堆叠原始错误句：逐条显示「备份中 / 本机中」内容、时间和标签；有冲突时「导入」必须 disabled，修改时区平移后实时重算，全部消解才允许提交。
...
7. 「···」更多菜单里存储、导入、分享、摘要、复制均可用；iPhone/iPad 存储优先打开文件分享面板，用户取消不下载，桌面仍直接下载；分享依次验证文件→文本→下载，无能力会下载、用户取消不下载；导入先预检，冲突逐条对照备份/本机内容且禁用提交，平移修改实时重算，冲突整批不写。从「更多」下钻进入标签设置/说明/导入检查后，取消、保存、Esc、点遮罩都回到「更多」。
8. PWA 更新链路：改 `index.html` 后升 CACHE 号；冷启动和回前台会检查更新，旧页面出现“更新应用”且按钮不被 FAB 遮挡；只有点击后才加载新版，本机 `localStorage['timelog.v1']` 保留。离线预缓存失败时旧 worker 继续服务。
8a. Safari 同标签刷新：人为延迟 `src/app.js` 时，模块到达前仍显示上一帧的周期、hero、时间轴、FAB 和「使用第 N 天」，不得露出静态「回到今天」或空内容壳；首次访问无快照时仍走正常启动门闩。
9. 午夜后重开仍停在上次所看日期；历史日续记无右邻时结束显示 24:00，不漏到当前时间。
10. 日视图卡片（v48）：普通记录可编辑完整起止时间并实时预览前/本/后三段；今日尾段可在「至今/固定」间切换；切一刀覆盖内部/贴边/整段；删除确认覆盖接回/转未记录/移除计划，8 秒撤销遇跨标签修改会失效。触摸/笔左滑揭示 2×72px 编辑/删除轨道，纵向滚动、点空白和打开另一张会关闭；鼠标不触发手势，键盘 Enter/Space 仍点卡编辑。
