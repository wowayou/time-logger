...
当前版本：`timelog-v35` / manifest `version: "35"`。
...
- rail 拖拽红线：`touch-action: none` 只允许作用在 `.tl-handle` 把手上，段主体和页面必须保持正常滚动；默认 5min 吸附、拖动中横向右移进入 1min 精调；把手聚焦后 ↑↓=5min、Shift+↑↓=1min；拖界与键盘微调的落库必须经 `normalizeEntries` 单一出口；已发生记录的 `ts` 不再从编辑表单修改（计划条除外）。真·静轴动标：拖动过程中相邻两段高度必须保持完全静止（不跟手指实时变形），只有把手时间数字和浮动气泡随手指变化；松手落库后再让现存两段柔和过渡到新高度（沿用 `.seg-block` 既有 200ms 过渡），过渡 settle 完再整体 render。
...
10. rail：按住边界时间可拖、5min 吸附、右移精调 1min，拖动中页面不跟滚、相邻两段高度纹丝不动（只有把手数字和气泡在变）；松手后目标段柔和过渡到新高度，无回弹瞬跳；点段开编辑、点空隙开补录、点尾段开记一条；拖后统计立即跟着变。
11. 新建/编辑 sheet 点「做了什么」呼出键盘：sheet 头部（取消/完成）全程不被顶出视口，随键盘展开/收起连续滑动，无二次跳位或短暂裸露遮挡。
