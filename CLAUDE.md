...
当前版本：`timelog-v1.0.0` / manifest `version: "1.0.0"`。
...
| 版本 | 日期 | 变更 |
|---|---|---|
| v1.0.0 | 2026-09-10 | **版本格式改三段式 + 发布防护 + Web→Android 显式契约 + 状态治理**。① **版本号从单整数改为 `MAJOR.MINOR.PATCH`**（`v93` → `v1.0.0`，历史 tag 与 `docs/CHANGELOG.md` 的单整数条目保留不改）。digit-only 判据实际散在**七处**而非六处：除六锚点外，`project_audit.py` 还断言 CLAUDE.md 含 `| vN |` 这行 CHANGELOG 行——它由 audit 强制却不在 `bump_version.py` 的 ANCHORS 里，是第七个锚点；另有 `publish-site.yml` 的 tag glob `v[0-9]+`、android 仓 `sync_runtime.py` 的 `web_version.isdigit()` 硬失败、`build.gradle.kts` 的 `toInt()` 与 `versionCode = webVersion * 100 + androidRevision`、android `project_audit.py` 的 `web version: (\d+)` 正则。**tag glob 必须先就绪**：不匹配的后果不是报错，而是 `publish-site` 静默不触发、线上主页停在上一次发布（2026-08-10 实测过一次同形失败），同批还给 workflow 加了「tag 与 manifest 版本一致」校验，挡住错版发布。**数据兼容性与产品版本完全解耦**，这次迁移不动数据：存储键 `timelog.v1` / `timelog.config`、备份载荷 `version: 1` 都与产品版本无关，且 `exportMeta()` 不写 app 版本、`validateImportData` 不校验 app 版本——所以 v1.0.0 导出的备份能被 v93 读回，反向亦然。新增 `tests/v1_0_0_version_migration.spec.js` 把这条从「文字承诺」变成可执行判据（含跨版本备份互读、以及一条锁死「存储键/载荷 version 不得跟随产品版本改名」的反向哨兵）。**一处预期内的一次性行为**：`app.js` 的启动快照门 `snap.appVersion !== APP_VERSION` 会让升级后第一次冷启动判定 `rejected:version` 并退回正常启动路径——设计如此（旧 DOM 形态不能在新版 JS 下继续活着），不是回归。`bump_version.py` 改为三段式，支持 `--patch` / `--minor` / `--major` 自增，并按元组比大小（旧代码 `int(target) <= int(current)` 在三段式下会抛 `ValueError`）。② **Android release 改 fail-closed**：`build.gradle.kts` 原来在缺 `keystore.properties` 时**静默回退 debug 签名**，意味着一次「成功」的 `bundleRelease` 可能产出永远无法上架、且与已分发包签名不同的产物。现在缺配置/字段空白/密钥文件不存在时 release 直接失败，debug 构建不受影响。`versionCode` 与 web 版本解耦（三段式无法参与 `*100` 公式），改由 `app/release.properties` 显式声明并单调递增，下限锁 9302（既有派生值 9301 之上），audit 断言不倒退。`sync_runtime.py` 新增 `--release`：先完整预检（web 仓 clean、commit 可解析、版本合法、契约有效）**再**替换资产——旧顺序是先复制文件后查 Git 来源，失败时已经改了工作区；且旧代码在 `git` 调用异常时把 dirty 标记一起清空、把「查不出来」写成了「干净」。③ **Web→Android 契约显式化**：新增 `native-contract.json`（开发期文件，刻意不进 `sw.js` FILES，audit 有反向断言），登记 android 侧真实依赖的 4 个模块 15 个 export 与 6 个 DOM selector；两仓 audit 各自校验——web 侧验证声明的 export/selector 真实存在，android 侧验证自己 import 的东西都已登记（删掉契约条目不能绕过检查）。DOM 清单比初版计划多两个：`shell_browser_check.mjs` 另外依赖 `#timeline` 与 `a[href^="https://github.com"]`。④ **状态治理**：新增 `STATUS.md` 作为唯一当前状态入口（DONE/OPEN/BLOCKED + 跨仓 commit + Last Verified），`docs/HANDOFF.md` 从 49KB 收缩为阅读顺序与交接入口——它此前同时写着「v93 已完成」和「v91 已发布上线」，自相矛盾。**版本仪式锚点与最近八行 CHANGELOG 留在 CLAUDE.md 不动**：audit 断言 CLAUDE.md 必须含 `timelog-vN.N.N` 与 `| vN.N.N |`，`bump_version.py` 的第 5 锚点也在这里，搬进 STATUS.md 等于要么 audit 红、要么削弱两个脚本的判据；`doc_sync_check.py` 的 DENY 规则同样不动（只把 STATUS.md 加进非阻断提醒），削弱它会让版本仪式失去唯一的自动化保护，而失效方向恰好是静默放行。⑤ **主页 favicon + 首屏文案**：四页此前**都没有 favicon 声明**，只有 hero `<img>`。四页相对深度各不相同（`./app/`、`../app/`、`../app/`、`../../app/`），audit 按页解析并核对目标文件真实存在，不只 grep `favicon`。首屏改为「不用记得先按开始／做完再补记」——明确是**用户主动补记**，不暗示自动还原一天。⑥ 顺带：`.gitignore` 补 `*:Zone.Identifier`（Windows 下载伴生文件，`.mp4` 本身已被忽略、这个没有，会直接卡住 `--release` 的 clean 预检）；CLAUDE.md CHANGELOG 表回到自己写的「最近八个版本」（实际已漂到十行），`docs/CHANGELOG.md` 归档 v86/v85/v84。零新增运行时资产（`FILES` 不变）。 |
