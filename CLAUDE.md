...
当前版本：`timelog-v50` / manifest `version: "50"`。
...
7. 「···」更多菜单里存储、导入、分享、摘要、复制均可用；iPhone/iPad 存储优先打开文件分享面板，用户取消不下载，桌面仍直接下载；分享依次验证文件→文本→下载，无能力会下载、用户取消不下载；导入先预检并显示导入/跳过/冲突汇总，冲突整批不写。从「更多」下钻进入标签设置/说明/导入时区平移后，取消、保存、Esc、点遮罩都回到「更多」。
