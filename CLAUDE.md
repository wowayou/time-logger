...
当前版本：`timelog-v60` / manifest `version: "60"`。
...
- Header 站点标识旁显示纯本地「使用第 N 天」；首次初始化写入 `timelog.firstUsedDate`，已有记录的老用户以最早本机记录日期迁移，之后不因联网、版本更新或导入更早历史而倒拨。`firstUsedDate` 随完整备份导出/导入（v60），使删主屏 PWA 重装或换设备后 N 能接上；导入只允许把起点往**更早**挪且拒绝未来日期，保证 N 单调不减。静态壳的「使用第 1 天」由 `styles.css` 的 `body:not(.app-ready):not(.boot-restored) .usage-day{visibility:hidden}` 挡住，不得让它在冷启动露出。
...
冻结期内**上述 28 天 gate 不是当前门槛，本冻结才是**（决策见 `docs/decisions.md` D9，执行边界见 `docs/dogfood-freeze-handoff.md`）：
