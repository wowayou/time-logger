...
> v1–v93 的历史条目已移至 `docs/CHANGELOG.md`（2026-07-25，D15 额度治理：本文件是三个模型每次会话都要整读的「法律」，历史流水不必每次付费）。
> **红线、边界、仪式条文全部留在本文件**，只有已归档的版本流水外移。本表保留最近八个版本。

| 版本 | 日期 | 变更 |
|---|---|---|
| v1.4.2 | 2026-09-24 | **修复 v1.4.1 验收的两项界面遗留**。① 合并提示改用独立的 `.cfg-merge-actions`，只在该容器清除取消按钮的下边距，保持合并两键等高；默认标签预览的两键恢复原有 10px 下边距。② 配置暂存快照单独记录草稿所在组 `groupBucket`，局部重渲按原组和原顺序回填，所选桶仍由控件单独保留；与已有行一致，保存前改桶不跳组，保存并重开后按实际桶归组。存储结构与保存事务不变，零新增运行时资产。扩展默认预览间距断言，新增维持→偏航、偏航→维持两条回归，覆盖设为当前、预览、取消、应用及最终保存；修复前双引擎六处判据均按预期红灯。`tests/tag_config_staging.spec.js` 现共九条，双引擎 18 条均一次通过，原合并按钮等高用例继续守护。全量 514 条：**512 passed / 2 flaky / 0 failed**（7.0 分钟）；两次重试分别是 Chromium 等待启动完成超时、WebKit 区间结束时间预览未更新，均在自动重试后通过，针对这两条关闭重试、双引擎各重复十次 **40/40 通过**。audit / smoke / typecheck / diff 检查通过。归档 v93，规范表保留最近八版。 |
| v1.4.1 | 2026-09-24 | **标签高级设置的并发写正确性补强 + 合并提示的可退出口（R5）**。三处收口，无功能扩张。① **局部动作的 CAS 基线不再现读**：`setCurrentMainline`／`applyLocaleDefaults` 原先在动作发生时 `loadConfigSnapshot()` 现读 raw 当基线——但 sheet 打开期间另一标签页若写过 config，现读值就是对方写的那份，用它当基线＝把对方的写入当成自己的起点，静默覆盖。改为用**打开 sheet 时**捕获的 `configRawAtOpen`；`saveConfigChecked` 成功时回传落库后的 normalized raw，调用方据此把基线推进到本次写入后的值（否则接着的局部动作拿旧基线再比一次会误判并发）。`openFormSheet` 只在 `prevMode !== 'config'`（真正从「更多/高级」下钻新开）时刷新基线，局部动作的原地重渲（`prevMode === 'config'`）不动它。② **未保存编辑熬过局部重渲**：`captureConfigEdits`／`restoreConfigEdits` 在「设为当前」「添加默认标签」等原地重渲整张 sheet 前抓下别处未保存的改名/改桶/longOk/待删除/草稿行，重渲后逐字回填（按 `originalName` 逐字匹配，存量 `sleep`/`Sleep` 并存不撞行）；重渲后已有记录的行不再挂删除按钮，待删除态遇此直接跳过、不留撤不掉的悬空态。③ **合并提示给显式「先不合并」出口**：破坏性合并不再是单一裸按钮——`showMergePrompt` 摆出中性「先不合并」（左）+ danger 色「合并」（右）两键，`dismissTagMerge` 只收起提示、不动数据并把焦点还给出问题那行的输入框。新增 i18n 键 `config.mergeCancel`（zh「先不合并」/ en "Not now"）。零新增运行时资产（`FILES` 不变，样式补一条 `.cfg-defaults-actions .cell-action{margin-bottom:0}` 抹平 cell-action 自带下边距造成的两键错峰）。判据：`tests/tag_config_staging.spec.js` 七条双引擎（设为当前保暂存含待删除态 / 预览不抹改名 / 合并提示双键+焦点归还 / ④设为当前跨页 CAS 拦截 / ⑤预览后保存跨页 CAS 拦截 / ⑥单页预览+应用暂存存活 / ⑦应用路径跨页 CAS 拦截），⑦附 P35 红灯（`applyLocaleDefaults` 基线改回现读 → 只有⑦红）。 |
| v1.4.0 | 2026-09-22 | **日视图「现在」条：一眼看到「正在做：X · 已 Ymin」（R2 方案 1 最小落地，gate 已由维护者裁定解锁）**。此前「进行中」的计时能力其实已在：时间轴最后一条已发生段用 `dur.ongoing`＝「已 Ymin」标进行中，FAB 副文案也报「续 X 起 · 已 Ymin」——**缺口只是「在做什么」这一眼**：内容藏在长列表最底部（日视图不自动滚到 now），顶部 hero 只报四桶分钟数、不报当前活动。本版在 hero 结论与时间轴之间加一条只读状态条 `#now-strip`：**仅**日视图看今天、且尾段是**非空 what 的已发生段**（真正「正在做某事」）时显示，点整条＝编辑该段（复用 `start-edit`／`data-id`，走既有点击委托）。未记录尾段不显示本条——FAB「续 X 起」已是同一入口，不做三重冗余。**纯展示**：把既有派生数据（`day.timeline` 里 `isOngoing` 那条的 `what`/`mins`/桶色）前置，不新增数据字段、不加真计时器、不碰统计与写路径；左侧桶色竖脊沿用 `.entry` 的 `data-b→--rail` 语法。切到周/月/年或非今天即隐藏（`renderNowStrip([], false)`）。新增 i18n `nowStrip.label`/`nowStrip.doingAria`（zh/en 对等）；英文用 "Now doing"，桶名沿用 Drift、不触 `Leak/Waste/Distraction/Unproductive/Wasted` 禁词。零新增运行时资产（`FILES` 不变，样式进既有 `styles.css`）。判据：`tests/v1_4_0_now_strip.spec.js` 双引擎（有进行中已发生段则显示且含 what+已 Ymin+桶色 / 点条打开该段编辑 / 未记录尾段不显示 / 周月年视图隐藏 / 零 pageerror）。 |
| v1.3.0 | 2026-09-22 | **「更多」里的时间拨号盘分析页（周期对比 + 趋势，只读）**。新增一个电话拨号盘隐喻的可视化页：顶部结论区 + 3 列圆盘拨号键（前 4 键＝四桶、其余按时长降序续排标签）+「接通」键复制本期摘要。挂在「更多」sheet 里排在「复制当前视图摘要」同组之后（`data-action=open-analytics`，`mode:'analytics'` 走既有 `openFormSheet` 二级页与返回栈，非新视图，不动 day/week/月/年 主视图与 native-contract selector）。**逻辑层（`stats.js` +164 / `time.js` +33，纯函数、config 由调用方注入、不碰 DOM/localStorage）**：`comparePeriods`（当期 vs 上期**同步进度**对比——进行中周期只跟上期「同样走到这个点」的裁剪区间比，不拿半周比整周；覆盖率按**已历天数**当分母而非整期）、`periodTrend`（近若干**已完成**同类周期的桶走向，可比周期 <3 判 `insufficient`、不编方向；`flat` 死区 max(30min,10%)；`runLength` 只在方向为 up/down 时计数）、`tagMinutes`、`elapsedMatchedRange`/`previousPeriodRange`。**修了一个自查出来的真 bug**：`elapsedMatchedRange` 早期只对周视图正确（皆 7 天），月/年当期已完成且短于上期时（2月 vs 1月、非闰年 vs 闰年），平移当期长度会漏掉上期尾几天——改为「当期已完成即整期对整期」，加了 `confirm_logic_smoke.py` 月/年回归断言。**UI 层（`ui.js` +163 渲染、`sheet_controller.js` 建模+交互）**：结论区把 `comparePeriods` 的 delta（可比才报带符号增减，欠覆盖报「记录不足」）与 `periodTrend` 的方向/连涨期数/sparkline 摆出来；点某键切换结论区、再点回主线；周/月/年切换就地重渲不整层关。新增 i18n `analytics.*` + `more.analytics`（zh/en 对等，626→上调后仍对等）。**只读**：不写记录、不改 config、无支付/埋点/网络。零新增运行时资产（`FILES` 不变，样式进既有 `styles.css`）。判据：`tests/v1_3_0_analytics_dialpad.spec.js` 5 条（入口打开+渲染+零 pageerror / 默认主线+走高趋势 / 区间切换就地重渲 / 选键往返 / 接通复制本期摘要）双引擎绿；`confirm_logic_smoke.py` 周期对比与趋势断言 + 月/年整期回归；`v84_polish` 行数 10→11。 |
| v1.2.1 | 2026-09-21 | **导入预览新增组弱化呈现（B）**——纯 UI/文案改动，无逻辑变更。v1.2.0 的逐条清单让部分用户误以为新增需逐条确认，实际上新增/已跳过从不拦「导入」按钮（`blocked = conflicts.length > resolvedCount`，只有冲突才需人工决策）。本版：新增组前置一句安心文案（`io.additionsAutoNote`：新记录随导入一次性全部加入、无需逐条确认、展开仅供核对）；去掉新增组主线色强调边框、组标题降为 muted 小字，让汇总行当主角，消除“像待办勾选表”的错觉。标题措辞改为「新增 {n} 条（可展开核对）」。冲突逐条决策、CAS 写入、additions/skippedEntries 字段全部一字不改。零新增运行时资产（`FILES` 不变）。判据：`tests/v1_2_0_import_preview.spec.js` 加安心文案断言。 |
| v1.2.0 | 2026-09-21 | **导入预览逐条化：新增 / 已跳过明明白白摆出来**（D13 ③“AI/自动化不得惄悔改原始时间线”的透明度硬化，非功能扩张）。之前导入检查只把「可导入 N 条 · 已存在跳过 M 条」当一行计数摆出，用户无从知道到底会往时间线里加什么、跳过了什么（只有冲突项有逐条卡片）。本版把 `preflightImportedEntries` 的 `additions`（新增）与 `skippedEntries`（完全相同、已存在）从内部计数升级为**导出字段**（`mergeImportedEntries` 的 `...plan` 展开自然透传；有冲突时也照样报出，因为非冲突新增/跳过不依赖冲突如何解决）。导入检查 sheet 新增 `data-role=import-detail` 容器，`io_actions.paintImportDetail` 把两组渲成可展开 `<details>` 清单（新增组边框用主线色、跳过组置灰，默认收起不撑爆矮视口）；每条显示时间 · #标签（· 计划）+ 内容。冲突区与确认/确认按钮逻辑一字不改（仍逐条保留本机/使用备份/合并文字，全部处理后才能写入）。新增 i18n 键 `io.groupAdditions`/`io.groupSkipped`/`io.itemMeta`（zh/en 对等）。零新增运行时资产（`FILES` 不变）；导出侧本就是完整备份（按 ts 升序、含 config/meta/firstUsedDate）本次不动。判据：`tests/v1_2_0_import_preview.spec.js`。 |
