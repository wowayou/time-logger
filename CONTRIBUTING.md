...
时间尺（Eigentime）是面向个人的时间记录与复盘工具：本地优先、零后端、零运行时依赖的单页 PWA。它的价值来自克制——大多数"能加的功能"都不该加。所以本指南的核心不是代码风格，而是一份**"这个改动该不该做"的判断清单**。
...
1. **过了门槛吗？** 功能扩张须按 `CLAUDE.md`「v2 锁死 & 别镀金」评审：累计 **28 天真实记录 + 求职有实质进展**只允许重新评审，不自动开工；已批准的例外不代表其它方向解锁。当前状态看 `STATUS.md`。
   - **最大风险 = 用打磨工具逃避面试推进。** 每次"再加个功能"冒出来，先问：目标公司的约面今天推进了吗？

2. **契合核心模型吗？** 时间尺的骨架是**点存储 + 区间 UX + 4 桶标签（主线 / 维持 / 偏航 / 未记录）+ 本地自然日 00:00 硬边界 + 本地优先**。撞这些前提的想法（时区转换、跨天汇总覆盖每日独立统计、账号、后端）默认不做。

3. **不破铁律吗？** 无运行时依赖 / 无构建 / 原生 ES modules。运行时文件只能相对路径导入本项目模块，禁止引入打包器、框架、npm 运行时依赖。npm 只能放开发期测试工具。

4. **归哪个模块？** 按 `CLAUDE.md`「模块边界」落位；本页不复制模块清单，以免新增模块后两处漂移。

5. **动了运行时文件吗？** 按 `CLAUDE.md`「当前版本」走版本仪式：用 `python3 scripts/bump_version.py <major.minor.patch>` 联动六处锚点，手写 CHANGELOG；增减资产时同步缓存与审计清单。纯文档改动不升运行时版本。
...
每次改完，提交前至少跑全以下检查（等价于 `CLAUDE.md`「改动自测清单」）：
...
npm run typecheck                      # 纯逻辑模块的 JSDoc 类型检查
...
- 正式版本推 `main` 后建并推同版本 tag（如 `v1.4.2`），再建/更新同版本 GitHub Release，简短列用户影响 / 内部治理 / 验证结果，不贴真实数据或截图。
