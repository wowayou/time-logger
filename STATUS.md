...
- **v1.1.0 自愿支持入口的落地页/README 链接已随 v1.4.0 上线**：`site/` 中英两页 footer + `README.md`「支持作者（自愿）」小节各加一条指向 `https://eigentime.org/support?from=time-logger`（逐字复用已登记 `from`、不分 locale）。线上已复核 zh footer「支持作者」、en footer "Support the author" 可见。**⚠ 与应用内链接同处已知死链窗口**（见 OPEN：无前缀 `/support` 仍 404），收口动作在网站侧。
