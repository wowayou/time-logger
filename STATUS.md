...
- **v1.3.0 发布收尾**（本批 AI 完成到「四闸全绿 + 双引擎全量通过、本地改动就绪（未 commit）」为止）：待维护者审逻辑与 UI 后 → commit → push main → tag `v1.3.0` → Release → publish-site → 线上复核 CACHE `timelog-v1.3.0`。以实际 git/gh 记录为准。
- **v1.2.1 发布收尾**（本批 AI 完成到「四闸全绿 + 全量 478 passed、本地 commit 就绪」为止）：push main → tag `v1.2.1` → Release → publish-site → 线上复核 CACHE `timelog-v1.2.1`。以实际 git/gh 记录为准。
