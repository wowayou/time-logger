...
**Last Verified：2026-09-24 · web `v1.4.1`（标签高级设置并发写保护、局部重渲保留未保存编辑、合并提示可取消；**已发布上线**——main `579271f` + tag `v1.4.1` + Release + publish-site 绿灯 + 线上复核 CACHE `timelog-v1.4.1`、manifest `1.4.1`；audit / smoke / typecheck / diff 检查通过 + 全量 test:ui 510 passed（chromium/webkit 255+255，0 failed、0 flaky）） · android `aab025d`（已推送，未发版） · 核验人：AI 代理**
